...
window.AdminView = {
  ...
  _dashboardHTML: function () {
    return '' +
      ...
      '        <div class="field span-2"><label>رابط الفيديو * (يوتيوب أو جوجل درايف)</label>' +
      '          <input id="f-youtube" class="input" required placeholder="https://www.youtube.com/watch?v=... أو https://drive.google.com/file/d/.../view"></div>' +
  ...
  _overviewHTML: function () {
    ...
    var recent = v.slice(0, 5).map(function (x) {
      return '' +
        ...
        '  <img src="' + UI.thumbnail(x.youtubeUrl) + '" alt="" loading="lazy" onerror="this.src=\'./imgs/pr2/1.jpg\'">' +
  ...
  _onSave: async function (e) {
    ...
    /* التأكد أن الرابط يوتيوب أو جوجل درايف يدعم التشغيل */
    if (!App.Media.detect(data.youtubeUrl)) {
      UI.toast('الرابط غير مدعوم — استخدم رابط يوتيوب أو جوجل درايف', 'error');
      return;
    }
